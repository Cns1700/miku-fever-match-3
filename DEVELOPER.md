...
**Gotcha this already caused once**: `updateBlossomBlastNotice()` can't tell "nothing has been placed yet" apart from "everything placed has already been cleared" — both look identical from a live scan (`countActiveDetonators() === 0`). `specialBlossomBreeze()`'s own placement loop can, in principle, come up with zero picks if the `>= 3` spacing rule can't be satisfied anywhere (an unusually obstacle-crowded board), and the original version of this had no guard for that — the special would run, place nothing, and `updateBlossomBlastNotice()` would immediately read that as "already done" and release the mana lock, which looked exactly like the special doing nothing at all (reported as: mana fills, player activates, no detonator ever appears on the board, bar just drops back to 0). Fixed two ways: `BLOSSOM_BLAST_MAX_DETONATORS` dropped from 4 to 2 (a much easier bar to clear geometrically — two mutually-`>=3`-apart cells are almost always findable even on a crowded board, four occasionally weren't), and `specialBlossomBreeze()` now guarantees at least one placement (ignoring the spacing rule only for that single last-resort pick) whenever any open cell exists at all, so this failure mode is closed regardless of board state.
